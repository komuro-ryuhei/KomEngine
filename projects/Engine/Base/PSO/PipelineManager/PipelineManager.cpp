...
void PipelineManager::ShaderCompile(const std::string& objectType) {
	...
	if (objectType == "object3d") {
		...
		// Shaderをコンパイルする
		vsBlob = compiler_->CompileShader(L"./Resources/shaders/CopyImage.VS.hlsl", L"vs_6_0", compiler_->GetDxcUtils(), compiler_->GetCompiler(), compiler_->GetIncludeHandler());
		assert(vsBlob != nullptr);
		psBlob = compiler_->CompileShader(L"./Resources/shaders/CopyImage.PS.hlsl", L"ps_6_0", compiler_->GetDxcUtils(), compiler_->GetCompiler(), compiler_->GetIncludeHandler());
		assert(psBlob != nullptr);
	}
}
